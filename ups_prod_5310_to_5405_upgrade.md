...
```bash
cpd-cli manage case-download --components=${COMPONENTS} --release=${VERSION} --patch_id=${PATCH_ID}
```

Remember to download the CASE package for the ibm_events_operator component as well
```bash
cpd-cli manage case-download --components=ibm_events_operator --release=${VERSION} --patch_id=${PATCH_ID}
```
...
```bash
oc apply -f /root/cpd-cli-workspace/olm-utils-workspace/work/cluster_scoped_resources.yaml --server-side --force-conflicts
```
...
**IMPORTANT**: Scale down the watsonx-ai-ifm operator before starting Orchestrate upgrade
```bash
...
After applying Hotfix1, monitor the deployments for Orchestrate and ensure that all of the relevant pods are able to start properly
```bash
...
After Watson Speech was upgraded to 5.3.1.0, a hot fix was applied to various image digests in the speech CR
...
**Note**: Confirm that the these hot fix 'digest' sections are removed in the speech CR before proceeding with the Watson Speech upgrade
...
Re-run HF1 after WxO and WA are fully reconciled to address this issue
